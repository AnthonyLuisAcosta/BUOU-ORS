    ...
    <div>
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
			...
			<div class="flex items-center justify-end px-3 py-4">
                <a href="{{ route('registrar.subjects.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest text-gray-800 shadow-md bg-sky-200 hover:bg-sky-400 hover:text-gray-200 disabled:opacity-25 transition ease-in-out duration-150">Add Subjects</a>
            </div>
					...
					<tbody>
						@foreach($subjects as $subject)
								...
								<td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
									<a href="{{ route('registrar.subjects.show', $subject->id) }}" class="text-blue-600 hover:text-blue-900 mb-2 mr-2">View</a>
									<a href="{{ route('registrar.subjects.edit', $subject->id) }}" class="text-indigo-600 hover:text-indigo-900 mb-2 mr-2">Edit</a>
									
									<!--DELETE BUTTON-->
									<div id="{{$subject->id}}" class="modal">
										<p>Are you sure you want to delete this subject?</p>
										<div class="text-right">
											<form class="inline-block" action="{{ route('registrar.subjects.destroy', $subject->id) }}" method="POST">
